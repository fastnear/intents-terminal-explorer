...
fn parse_args() -> Args<'static> {
    ...
    while i < it.len() {
        match it[i].as_str() {
            "--edges" => {
                i += 1;
                edges_path = Box::leak(it[i].clone().into_boxed_str());
            }
            "--nodes" => {
                i += 1;
                nodes_path = Some(Box::leak(it[i].clone().into_boxed_str()));
            }
            "--skip-header" => {
                skip_header = true;
            }
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: compcount --edges <file> [--nodes <file>] [--skip-header] [--directed]"
                );
                std::process::exit(0);
            }
            x => {
...
fn tokenize(line: &str) -> Vec<&str> {
    line.split(
        |c: char| c.is_whitespace() || c == ',' || c == ';' || c == '\u{0001}', /* ctrl-A */
    )
...
fn main() -> io::Result<()> {
    ...
    } != 0
    {
        ...
        let v_lab = t[1];
        if u_lab == v_lab {
            // self-loop contributes degree but not components
